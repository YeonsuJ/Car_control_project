...
목적
1. 불필요한 데이터 차단 : 모든 노드가 브로드캐스트 버스를 공유하므로, 관심 없는 메시지를 차단하여 CPU 낭비를 줄임
2. 실시간성 확보 : 필요한 메시지에만 반응하므로, 빠르고 정확한 처리 가능
3. 시스템 안정성 향상 : 수신 FIFO가 가득 차는 현상(오버플로)을 방지
4. 멀티 노드 환경 대응 : 여러 노드가 각각 다른 메시지 ID를 처리할 때 필요

설정 방법
CAN 필터 설정은 `CAN_FilterTypeDef` 구조체를 이용하며, 주요 파라미터는 다음과 같다:
```c
CAN_FilterTypeDef sFilterConfig;
sFilterConfig.FilterActivation = CAN_FILTER_ENABLE;
sFilterConfig.FilterFIFOAssignment = CAN_FILTER_FIFO1; // FIFO0 또는 FIFO1
sFilterConfig.FilterMode = CAN_FILTERMODE_IDMASK;      // ID 마스크 방식
sFilterConfig.FilterScale = CAN_FILTERSCALE_32BIT;     // 32비트 단일 필터 사용

// 수신하고자 하는 ID 지정 (예: 0x6A5)
sFilterConfig.FilterIdHigh = 0x6A5 << 5;  // 11비트 표준 ID를 상위 정렬
sFilterConfig.FilterIdLow = 0;

// 필터 마스크 설정 (모든 비트 비교할 경우: 0x7FF << 5)
sFilterConfig.FilterMaskIdHigh = 0x7FF << 5;
sFilterConfig.FilterMaskIdLow = 0;
```
ID 방식 선택
- CAN_FILTERMODE_IDMASK: 마스크를 이용해 비교 (가장 일반적)
- CAN_FILTERMODE_IDLIST: 최대 4개의 ID만 수신하도록 고정 설정

FIFO 선택
- 수신 데이터가 FIFO0 또는 FIFO1 중 어디로 들어갈지 선택
- 후속 처리 함수(인터럽트 콜백 등)에서 일치되게 사용해야 함
