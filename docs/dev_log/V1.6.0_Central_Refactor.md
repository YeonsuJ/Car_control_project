...
> [V1.6.0_Controller_Refactor.md 참고](./V1.6.0_Controller_Refactor.md)
...
uint8_t RxData[8];
volatile uint8_t can_distance_signal = 0; // 수신된 거리 신호 상태 저장
...
// ▶ 필터 설정: 0x6A5 ID 수신 허용
void CAN_Filter_Config(CAN_HandleTypeDef *hcan_ptr)
{
    sFilterConfig.FilterActivation = CAN_FILTER_ENABLE;
    sFilterConfig.FilterFIFOAssignment = CAN_FILTER_FIFO1;
    sFilterConfig.FilterMode = CAN_FILTERMODE_IDMASK;
    sFilterConfig.FilterIdHigh = 0x6A5 << 5;              // 수신 ID
    sFilterConfig.FilterIdLow = 0;
    sFilterConfig.FilterMaskIdHigh = 0x7FF << 5;          // 전비트 일치
...
// ▶ 수신 인터럽트 콜백 함수 (FIFO1)
// 수신된 거리값이 1이면 햅틱 GPIO ON, 0이면 OFF
void HAL_CAN_RxFifo1MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO1, &RxHeader, RxData);

    // 햅틱 제어 핀 (예: PB10) 상태 설정
    if (RxData[0] == 1)
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, GPIO_PIN_SET);
    else
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, GPIO_PIN_RESET);

    // 거리 상태 저장 (RF 모듈 ACK 응답 등에 사용됨)
    can_distance_signal = RxData[0];
}
```
- CAN 필터 설정 (ID 매칭 기반 수신 허용)
    - CAN_Filter_Config() 함수는 표준 ID 0x6A5만 수신하도록 필터를 구성
    - 마스크 방식(IDMASK)을 사용하여 11비트 전체 일치 조건으로 설정
    - 수신 메시지는 FIFO1으로 들어오게 설정되며, 해당 FIFO에서 수신 이벤트 발생 시 인터럽트가 발생
    - 목적: 다른 CAN 메시지는 무시하고, 초음파 거리 감지 결과(ID: 0x6A5)만 수신하여 성능 및 안정성 확보

- CAN 수신 인터럽트 처리 (FIFO1 기반)
    - HAL_CAN_RxFifo1MsgPendingCallback() 함수는 수신 인터럽트가 발생할 때 호출
    - 수신된 메시지의 RxData[0] 값(0 또는 1)을 기반으로 햅틱 진동 유무를 결정
    - 동시에 이 값을 전역 변수 can_distance_signal에 저장하여 RF ACK 응답 등 타 모듈에서 참조 가능하도록 함
    - 목적: 거리 위험 조건을 실시간으로 햅틱 출력에 반영하고, RF 응답에도 사용할 수 있도록 전역 상태 공유

- 모듈 초기화 및 시작
    - CANHandler_Init() 함수는 HAL_CAN_Start()로 CAN 통신을 시작하고, 필터를 설정한 뒤 인터럽트를 활성화
    - 목적: 메인 루프에서 별도로 CAN을 수동으로 다룰 필요 없이, 자동 수신 및 처리 구조 완성
