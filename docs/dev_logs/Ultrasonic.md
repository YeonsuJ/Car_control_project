...
## 프로젝트 활용 방안
초음파 센서는 차량 전방의 장애물을 감지하기 위해 사용되며, 측정된 거리 데이터는 핸들에 연결된 진동모터 기반 충돌 방지 경고 시스템에 전달되어 운전자에게 피드백을 제공하는 데 활용된다.
...
#### 인터럽트 기반 타이머 인풋 캡쳐를 활용한 초음파센서 거리값 출력
### 1. 초기 코드 방식의 문제점
