...
현재 구조는 RF 수신, 모터 제어, CAN 통신 등 다양한 작업을 하나의 메인 루프에서 순차적으로 수행한다. 이 방식은 기능이 증가할수록 특정 작업이 다른 작업의 실행을 지연시키는 문제가 발생할 수 있다. RTOS를 도입하면 이러한 문제를 해결하고 다음과 같은 효과를 기대할 수 있다:

- 실시간 응답성 확보 : 시간에 민감한 작업(RF 명령 수신, 모터 제어)과 덜 민감한 작업(상태 전파)을 각각 독립적인 **태스크(Task)**로 분리하고 우선순위를 부여할 수 있다. 이를 통해 모터 제어가 다른 기능에 의해 지연되는 현상을 원천적으로 방지하고, 운전자의 조작에 즉각적으로 반응하는 시스템을 구현할 수 있다.

- 안정적인 동시 처리 : RF 수신과 CAN 수신처럼 언제 발생할지 모르는 비동기 이벤트들을 **세마포어(Semaphore)와 메시지 큐(Message Queue)**를 통해 효율적으로 처리할 수 있다. CPU가 불필요한 대기(Polling) 없이 이벤트를 처리하게 되어 시스템의 전반적인 효율과 안정성이 크게 향상된다.

- 시스템 확장성 향상 : 각 기능이 독립적인 태스크로 모듈화되어 있어, 향후 PID 제어, 데이터 로깅, 배터리 모니터링 등 새로운 기능을 추가할 때 기존 코드에 미치는 영향을 최소화하며 쉽게 시스템을 확장할 수 있다.

### 단계별 적용 계획

#### 1단계: 태스크 분리 및 우선순위 설계
시스템의 핵심 기능을 2개의 독립적인 FreeRTOS 태스크로 분리하여 전환할 계획이다.

- `RFTask` (높은 우선순위): 조종기 RF 명령 수신, 주행 명령 해석, 모터 제어, CAN 피드백 처리 등 시스템의 핵심 제어 로직을 총괄하는 태스크를 설계한다. 사용자 조작에 대한 즉각적인 반응을 보장하기 위해 가장 높은 우선순위를 할당할 것이다.

- `CANTask` (보통 우선순위): RFTask로부터 처리된 차량 상태를 전달받아 CAN 버스로 전파하는 게이트웨이 역할을 수행하는 태스크를 설계한다. 제어 자체보다 시간 제약이 덜 엄격하므로 보통 우선순위로 설정하여 핵심 제어 로직을 방해하지 않도록 한다.

#### 2단계: 태스크 간 통신 및 동기화 구조 변경
태스크 간 데이터 교환 및 이벤트 처리는 RTOS가 제공하는 안전하고 효율적인 메커니즘을 통해 구현할 것이다.

- **인터럽트 처리** : RF 수신 인터럽트는 **세마포어(RFSemHandle)**를 통해 RFTask를 즉시 깨우고, CAN 수신 인터럽트는 **메시지 큐(CANRxQueueHandle)**에 데이터를 직접 전송하여 RFTask에 안전하게 전달하는 구조를 적용한다.

- **태스크 간 통신**: RFTask는 처리된 주행 명령을 또 다른 **메시지 큐(CANTxQueueHandle)**에 전송하고, CANTask는 해당 큐로부터 데이터를 수신하여 처리하는 방식으로 두 태스크의 동작을 완벽히 분리할 계획이다.
