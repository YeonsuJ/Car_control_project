...
현재 Sensor ECU는 while(1) 기반의 순차 처리(Superloop) 구조로 동작하고 있다. 이는 초음파 센서 트리거, 엔코더 RPM 계산, CAN 데이터 전송 등 모든 로직이 단일 흐름 안에서 순서대로 실행됨을 의미한다.
앞으로 시스템의 실시간성, 확장성, 안정성을 획기적으로 향상시키기 위해 FreeRTOS 도입을 목표로 시스템 구조를 고도화할 예정이다.
...
### RTOS 적용 시 기대 효과
- 실시간성 향상
    - 센서 데이터 수집과 CAN 전송 로직을 독립적인 Task 단위로 분리할 것이다.
    - **SensorTask**는 오직 10ms 주기에 맞춰 데이터를 정밀하게 수집하는 역할만 수행하고, **CANTask**는 데이터가 준비되었을 때만 깨어나 전송을 담당하게 된다.
    - 시간에 가장 민감한 SensorTask에 높은 우선순위를 부여하여 다른 작업에 의해 데이터 수집 주기가 밀리는 현상을 원천적으로 방지할 것이다.

- 시스템 확장성 향상
	- 향후 자가 진단, 상태 모니터링 등 새로운 기능이 추가될 때, 기존 로직을 수정할 필요 없이 새로운 태스크만 추가하면 되므로 구조적으로 깔끔하게 확장이 가능하다.
 	- 태스크 간 데이터는 **메시지 큐(Message Queue)**를 통해 안전하게 전달하여 데이터의 흐름을 명확히 하고, 인터럽트와 태스크가 공유하는 자원은 **크리티컬 섹션(Critical Section)**으로 보호하여 데이터 충돌 없이 안정적인 동작을 보장할 것이다. 

### 단계별 RTOS 적용 계획
- 1단계: 기능 분리 및 태스크화
	- `SensorTask`: 10ms의 정밀한 주기로 깨어나 초음파 센서를 트리거하고, 엔코더 값을 읽어 RPM을 계산한 뒤, 측정된 모든 데이터를 종합하여 내부 데이터 구조체에 저장하는 역할을 맡길 것이다.
	- `CANTask`: SensorTask가 측정을 마친 데이터를 읽어와 CAN 프로토콜 형식에 맞게 가공하고(e.g., 거리→비트마스크, RPM→2바이트 분리), 이를 전송하는 역할만 전담하도록 할 것이다.

- 2단계: 태스크 간 통신 구조 적용
	- SensorTask가 측정한 최종 데이터 패킷(SensorData_t)을 **메시지 큐(Message Queue)**를 사용해 CANTask로 전달하는 구조를 적용할 것이다.
 	- 이러한 이벤트 기반 구조를 통해 CANTask는 데이터가 있을 때만 동작하므로 CPU 자원을 효율적으로 사용하고, 두 태스크의 동작이 완벽히 분리(Decoupling)되어 시스템의 예측 가능성과 디버깅 용이성이 향상될 것이다.

- 3단계: 자원 보호 적용
	- 초음파 거리 값처럼 인터럽트 서비스 루틴(ISR)과 SensorTask가 공유하는 데이터에 대해서는, 데이터 읽기/쓰기 도중 충돌이 발생하지 않도록 **크리티컬 섹션(Critical Section)**으로 보호하여 데이터 무결성을 확보할 계획이다.
 	 이를 통해 태스크가 데이터를 읽는 도중에 인터럽트가 발생하여 값이 변경되는 위험을 방지할 것이다.
