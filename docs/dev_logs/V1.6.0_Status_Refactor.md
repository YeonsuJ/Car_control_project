...
앞으로 시스템의 실시간 반응성, 안정성, 확장성을 더욱 향상시키기 위해 FreeRTOS 도입을 통해 현재의 순차적 처리 구조를 멀티태스킹(Multi-tasking) 구조로 전환할 계획이다.

### RTOS 도입의 필요성 및 기대 효과

현재 구조는 CAN 수신 처리 → 상태 갱신 → 배터리 측정 → OLED 출력 순서로 동작한다. 이 구조는 간단하지만, CAN 메시지 수신 빈도가 높아지거나 향후 다른 기능이 추가될 경우 다음과 같은 문제가 발생할 수 있다.

- 처리 지연: OLED 업데이트와 같이 상대적으로 느린 작업이 수행되는 동안 중요한 CAN 메시지 처리가 지연될 수 있다.
- 기능 간섭: 모든 기능이 하나의 실행 흐름(Context)에서 동작하므로, 한 기능의 변경이 다른 기능에 예기치 않은 영향을 줄 수 있다.

RTOS를 적용하면 각 기능을 독립적인 **태스크(Task)**로 분리하여 이러한 문제를 해결하고 다음과 같은 효과를 기대할 수 있다.

- 실시간 응답성 확보: CAN 데이터 처리처럼 시간 제약이 중요한 작업에 높은 우선순위를 부여하여, 디스플레이 업데이트와 같은 다른 작업에 의해 실행이 방해받지 않도록 보장한다.
- 시스템 확장성 향상: 향후 자가 진단, 로그 전송 등 새로운 기능을 기존 코드에 영향을 주지 않고 독립된 태스크 단위로 안전하게 추가할 수 있다.
- 안정성 및 신뢰성 증대: 메시지 큐(Message Queue)를 통해 태스크 간 데이터 전송을 안전하게 관리하여 데이터 손실이나 오염을 방지하고, 시스템의 전체적인 안정성을 높일 수 있다.

### 단계별 RTOS 적용 계획

#### 1단계: 기능 분리 및 태스크화
가장 먼저, 현재 순차적으로 실행되는 주요 기능들을 역할에 따라 두 개의 독립적인 태스크로 분리할 것이다.

- CANTask: CAN 버스로부터 들어오는 모든 메시지를 수신하여 처리하고, 통신 상태를 진단하는 데이터 처리 전담 태스크로 설계한다.
- DisplayTask: CANTask가 처리한 최종 데이터를 기반으로 LED 및 OLED 디스플레이 출력을 총괄하는 사용자 인터페이스 전담 태스크로 설계한다. 배터리 측정 로직은 이 태스크 내에서 주기적으로 수행하도록 통합한다.

#### 2단계: 태스크 간 통신 구조 설계
분리된 태스크들이 데이터를 안전하고 효율적으로 주고받을 수 있도록 메시지 큐 기반의 통신 구조를 설계할 것이다.

- 인터럽트 → 태스크 통신: CAN 수신 인터럽트(ISR)는 수신 데이터를 직접 처리하지 않고, CANRxQueue라는 메시지 큐에 삽입한다. CANTask는 이 큐에 데이터가 들어올 때까지 대기하다가, 데이터 수신 시 즉시 처리하여 ISR의 작업 부하를 최소화한다.
- 태스크 → 태스크 통신: CANTask는 수집된 모든 데이터를 가공하고 통신 상태를 진단한 뒤, DisplayDataQueue라는 별도의 큐를 통해 DisplayTask로 최종 상태 정보를 전달한다. 이를 통해 데이터 처리 로직과 출력 로직을 완벽히 **분리(Decoupling)**한다.
